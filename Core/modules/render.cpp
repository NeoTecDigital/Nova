...
void NovaCoreLegacy::drawFrame()
    {
        ...
        // Use per-swapchain-image semaphores indexed by the acquired image index
        // This prevents the validation error about semaphore reuse
        VkResult result = vkAcquireNextImageKHR(
                            logical_device,
                            swapchain.instance,
                            UINT64_MAX,
                            current_frame().image_available[_frame_ct],  // Use frame-specific semaphore
                            VK_NULL_HANDLE,
                            &_image_index
                        );
        ...
        // Use the acquired image index for the render_finished semaphore
        // This ensures each swapchain image has its own presentation semaphore
        ...
        present.submit_info = {};
        VkSemaphore _wait_semaphores[] = { current_frame().image_available[_frame_ct] };  // Wait on frame-specific acquisition semaphore
        VkSemaphore _signal_semaphores[] = { current_frame().render_finished[_image_index] };  // Signal per-image presentation semaphore
