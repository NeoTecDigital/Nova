...
/**
 * NovaSDL - the ONLY window-system boundary in the Nova engine.
 *
 * Every call into libSDL2 that Nova makes lives in Core/nova_sdl.cpp, and that
 * translation unit is compiled into its own archive (target `NovaSDL`) rather
 ...
 *
 * Link `NovaSDL` from a target only if that target opens an SDL window.
 *
 ...
 * to NovaCore::createVulkanInstance produces an instance with no WSI support,
