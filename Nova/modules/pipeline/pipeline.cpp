#include "./pipeline.h"
#include "Nova/components/genesis.h"
namespace Nova {
...
Builder& Builder::shaders(VkDevice* logical_device, const std::string& vert_path, const std::string& frag_path)
    {
        ...
        // Only load shaders if paths are provided
        if (vert_path.empty() || frag_path.empty()) {
            report(LOGGER::INFO, "Pipeline::shaders() - No shader paths provided, skipping shader creation");
            return *this;
        }

        std::vector<char> _vert_shader_code = genesis::loadFile(vert_path);
        ...
        std::vector<char> _frag_shader_code = genesis::loadFile(frag_path);
